...
def all(filename,checkpoint=False):
	...
	log.print("Cation-Cation REGRESSION:")
	[pred_3,t_3],[slope_3,intercept_3] = regression(inter1,t,scaling=0.3)
	log.print("Anion-Anion REGRESSION:")
	[pred_4,t_4],[slope_4,intercept_4] = regression(inter2,t,scaling=0.3)
	log.print("Cation-Anion REGRESSION:")
	[pred_5,t_5],[slope_5,intercept_5] = regression(inter_inter,t,scaling=0.3)
	
	tot_coll=cat+ani+inter1+inter2+inter_inter	
	log.print("Sum of Total Components REGRESSION:")
	[pred_6,t_6],[slope_6,intercept_6] = regression(tot_coll,t,scaling=0.3)
	
	
	fig, ax = plt.subplots()
	ax.plot(t,cat,linewidth=1.3,label=f'msd cation D:{slope_1/6} pm^2/ps',color='blue',zorder=2)
	ax.plot(t_1,pred_1,linewidth=1,linestyle='dashed',color='blue',zorder=3)
	ax.plot(t,ani,linewidth=1.3,label=f'msd anion D: {slope_2/6} pm^2/ps',color='green',zorder=2)
	ax.plot(t_2,pred_2,linewidth=1,linestyle='dashed',color='green',zorder=3)
	ax.plot(t,tot_coll,linewidth=1.3,label=f'msd sum D: {slope_6/6} pm^2/ps',color='purple',zorder=2)
	ax.plot(t_6,pred_6,linewidth=1,linestyle='dashed',color='purple',zorder=3)
	ax.plot(t,inter1,linewidth=1.3,label=f'msd cation-cation D: {slope_3/6} pm^2/ps',color='brown',zorder=2)
	ax.plot(t_3,pred_3,linewidth=1,linestyle='dashed',color='brown',zorder=3)
	ax.plot(t,inter2,linewidth=1.3,label=f'msd anion-anion D: {slope_4/6} pm^2/ps',color='orange',zorder=2)
	ax.plot(t_4,pred_4,linewidth=1,linestyle='dashed',color='orange',zorder=3)
	ax.plot(t,inter_inter,linewidth=1.3,label=f'msd cation-anion D: {slope_5/6} pm^2/ps',color='red',zorder=2)
	ax.plot(t_5,pred_5,linewidth=1,linestyle='dashed',color='red',zorder=3)
	ax.legend()
